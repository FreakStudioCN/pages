...
class TM1637(object):
    def __init__(self, clk:Pin, dio:Pin, brightness:int=7):
        self.clk = clk
        self.dio = dio
    ...
    def write(self, segments : list, pos : int=0) -> None:
        if not 0 <= pos <= 5:
    ...
    def encode_digit(self, digit : int) -> bytes:
    ...
    def encode_char(self, char : str) -> bytes:
        o = ord(char)
        # 空格
        if o == 32:
            return _SEGMENTS[36]
