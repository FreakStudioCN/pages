...
class VL53L0X:
    ...
    def set_Vcsel_pulse_period(self, type, period_pclks) -> bool:
        ...
        if type == self.vcsel_period_type[0]:
            ...
            new_pre_range_timeout_mclks = self.timeout_microseconds_to_Mclks(self.timeouts["pre_range_us"],
                                                                             period_pclks)
            self._register(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI, int(
            ...
            new_msrc_timeout_mclks = self.timeout_microseconds_to_Mclks(self.timeouts["msrc_dss_tcc_us"],
                                                                        period_pclks)
            self._register(MSRC_CONFIG_TIMEOUT_MACROP, 255 if new_msrc_timeout_mclks > 256 else (
    ...
    def get_sequence_step_timeouts(self):
        ...
        self.timeouts["msrc_dss_tcc_mclks"] = int(
            self._register(MSRC_CONFIG_TIMEOUT_MACROP)) + 1
        self.timeouts["msrc_dss_tcc_us"] = int(self.timeout_Mclks_to_microseconds(
        ...
        self.timeouts["pre_range_us"] = int(self.timeout_Mclks_to_microseconds(
        ...
        self.timeouts["final_range_us"] = int(self.timeout_Mclks_to_microseconds(
    ...
    def set_measurement_timing_budget(self, budget_us) -> bool:
        ...
        if self.enables["dss"]:
            used_budget_us += 2 * \
                self.timeouts["msrc_dss_tcc_us"] + dss_overhead
        if self.enables["msrc"]:
            used_budget_us += self.timeouts["msrc_dss_tcc_us"] + msrc_overhead
        if self.enables["pre_range"]:
            used_budget_us += self.timeouts["pre_range_us"] + \
                pre_range_overhead
